...
export default function EditorPage() {
  ...
  const [projectName, setProjectName] = useState("My First Architecture");
  const [projectDesc, setProjectDesc] = useState("");
  const [workspaceProjectName, setWorkspaceProjectName] = useState("My First Architecture");

  const [prompt, setPrompt] = useState("");

  const handleCreateProject = (e: React.FormEvent) => {
    e.preventDefault();
    setWorkspaceProjectName(projectName);
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-base text-copy-primary overflow-hidden font-sans select-none">
      ...
      <main className="relative flex-1 bg-base min-w-0 transition-all duration-300">
        ...
        {/* Workspace Central Shell (Greeting & Prompt box) */}
          ...
          <div className="w-full max-w-xl p-8 rounded-3xl bg-surface/85 border border-surface-border shadow-2xl backdrop-blur-md flex flex-col gap-6 relative z-10">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-copy-muted leading-relaxed">
                Describe your desired system architecture (e.g., &quot;microservice platform with Kafka message broker and Redis caching&quot;). Our AI generator will map nodes and connections inside the collaborative canvas.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="default"
                    className="h-8.5 rounded-lg text-xs gap-1.5 bg-brand text-bg-base hover:bg-brand/90 font-medium"
                    onClick={() => {
                      if (!prompt) return;
      ...
      {/* 4. Dialog Pattern Proof (New Project Dialog) */}
        ...
        <DialogContent className="max-w-md bg-surface border border-surface-border rounded-2xl p-6 shadow-2xl">
          <form onSubmit={handleCreateProject} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="project-name" className="text-[11px] font-semibold text-copy-muted uppercase tracking-wider">
                  Project Name
                </label>
                <Input
                  id="project-name"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="project-desc" className="text-[11px] font-semibold text-copy-muted uppercase tracking-wider">
                  Description
                </label>
                <Textarea
                  id="project-desc"
            ...
            <DialogFooter className="mt-2 border-t border-surface-border pt-4 -mx-6 -mb-6 px-6 bg-subtle/30 rounded-b-2xl">
              ...
              <Button
                variant="default"
                type="submit"
                className="h-8.5 rounded-lg text-xs bg-brand text-bg-base hover:bg-brand/90 font-semibold"
              >
                Create Workspace
              </Button>
            </DialogFooter>
